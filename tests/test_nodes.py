...
async def test_approval_node_approve(mock_exists, mock_open, mock_markdown_to_pdf, mock_get_llm, mock_telegram_class):
    mock_telegram = AsyncMock()
...
async def test_execution_node_unexpected_exception(mock_screenshot, mock_telegram_class, mock_managed_browser):
    ...
    with patch("jobapply.nodes.execution.os.path.exists", return_value=True), \
