...
async def increment_page_node(state: JobApplyState) -> dict:
    ...
    if max_jobs is not None:
        print(f"📄 Moving to page {state['current_page'] + 1} (need {max_jobs - jobs_evaluated} more jobs)")
    else:
...
def build_graph() -> StateGraph:
    ...
    builder.add_node("search_node", search_node, metadata={
        "description": "LinkedIn job search via Playwright",
        "operations": ["CDP connect", "page navigation", "job card extraction"]
    })
    builder.add_node("select_next_job_node", select_next_job_node, metadata={
        "description": "Iterator/router for unseen jobs",
        "operations": ["job iteration", "pagination", "query advancement"]
    })
    builder.add_node("qualification_node", qualification_node, metadata={
        "description": "LLM-powered job fit evaluation",
        "operations": ["LLM scoring", "MongoDB dedup"]
    })
    builder.add_node("generation_node", generation_node, metadata={
        "description": "Cover letter + urgency check generation",
        "operations": ["cover letter LLM", "urgency check LLM", "file output"]
    })
    builder.add_node("approval_node", approval_node, metadata={
        "description": "Telegram human-in-the-loop for urgent edits",
        "operations": ["Telegram send", "Telegram wait"]
    })
    builder.add_node("execution_node", execution_node, metadata={
        "description": "Easy Apply automation with inline Q&A",
        "operations": ["browser navigation", "form filling", "Telegram Q&A", "submission"]
    })
    builder.add_node("notification_node", notification_node, metadata={
        "description": "Session summary via Telegram",
        "operations": ["Telegram send"]
    })
    builder.add_node("increment_page_node", increment_page_node, metadata={
        "description": "Increment page counter",
        "operations": ["state update"]
    })
    builder.add_node("increment_query_node", increment_query_node, metadata={
    ...
    builder.add_conditional_edges("select_next_job_node", route_select_next_job, {
    ...
    builder.add_conditional_edges("qualification_node", route_after_qualification, {
        "generation_node": "generation_node",
        "select_next_job_node": "select_next_job_node",
    })

    # generate → execute (fast path) or approval (rare edit path)
    builder.add_conditional_edges("generation_node", route_after_generation, {
    ...
    builder.add_conditional_edges("execution_node", route_post_processing, {
        "select_next_job_node": "select_next_job_node",
        "notification_node": "notification_node",
    })
...
def make_graph_config(run_id: str, tags: list[str] = None, metadata: dict = None) -> dict:
