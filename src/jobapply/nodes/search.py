...
from jobapply.utils.browser import managed_browser, get_randomized_delay
        ...
        async with managed_browser() as (browser, context):
            ...
            async with trace(
                ...
                try:
                    ...
                    print(f"[DEBUG] Scrolling to load all jobs...")
                ...
                for card in job_cards[:25]:  # Limit to 25 per page
                    try:
                        ...
                        if not job_id:
                            print(f"[DEBUG] Skipping card - no job_id")
                            continue
                        ...
                        if len(job_listings) >= unseen_limit:
                            print(f"[DEBUG] Already have enough unseen jobs in this batch ({len(job_listings)} >= {unseen_limit}), stopping card extraction.")
                            break
                        ...
                        fresh_card = await page.query_selector(f"li[data-occludable-job-id='{job_id}']")
                        if not fresh_card:
                        ...
                        for attempt in range(3):
                            ...
                            link_elem = await fresh_card.query_selector("a.job-card-container__link")
                            company_elem = await fresh_card.query_selector(".artdeco-entity-lockup__subtitle")
                        ...
                        if not all([link_elem, company_elem]):
                            print(f"[DEBUG] Job {job_id} - elements still not loaded after retries, skipping")
                            continue
                        
                        # Location is in the metadata wrapper
                        location_elem = await fresh_card.query_selector(".job-card-container__metadata-wrapper span")
                        ...
                        if len(job_listings) >= unseen_limit:
                            print(f"[DEBUG] Reached target unseen jobs limit ({unseen_limit}), stopping extraction.")
                            break
                        ...
                        print(f"[DEBUG] Error extracting job {job_id if 'job_id' in locals() else 'unknown'}: {str(e)}")
                        import traceback
                ...
                if run_tree:
                    run_tree.metadata.update(get_search_metadata(query, page_num, len(job_listings)))
