...
def session_application_count(state: JobApplyState) -> int:
    ...
    if state.get("dry_run"):
        count += sum(
            outcome.get("status") == "dry_run"
            for outcome in state.get("application_outcomes", [])
        )
    return count
